"use client";

import { useState } from "react";
import Image from "next/image";
import styles from "./page.module.css";
import deos from "@/lib/data/deos.json";
import WhatsAppOrderModal from "../components/whatsappordermodal/whatsappordermodal";

export default function Deodorants() {
  const [selectedProduct, setSelectedProduct] = useState(null);
  const [isModalOpen, setIsModalOpen] = useState(false);

  const formatPrice = (price) => `${price.toLocaleString("fr-FR")} FCFA`;

  const handleOrderClick = (deo) => {
    setSelectedProduct(deo);
    setIsModalOpen(true);
  };

  const handleCloseModal = () => {
    setIsModalOpen(false);
    setSelectedProduct(null);
  };

  return (
    <main className={styles.main}>
      <section className={styles.container_title}>
        <h1 className={styles.title}>
          Découvrez nos <span>déodorants</span>
        </h1>
      </section>

      <section className={styles.container}>
        <div className={styles.cont}>
          {deos.map((deo) => (
            <div className={styles.box} key={deo.id}>
              <div className={styles.imageContainer}>
                <Image
                  src={deo.image}
                  alt={deo.name}
                  fill
                  sizes="(max-width: 480px) 45vw, (max-width: 769px) 30vw, (max-width: 1200px) 22vw, 260px"
                  className={styles.img}
                />
              </div>

              <div className={styles.content}>
                <h3 className={styles.box_title}>{deo.name}</h3>
                <p className={styles.price}>{formatPrice(deo.price)}</p>

                <button
                  className={styles.button}
                  onClick={() => handleOrderClick(deo)}
                >
                  Commander
                  <span>→</span>
                </button>
              </div>
            </div>
          ))}
        </div>
      </section>

      <WhatsAppOrderModal
        product={selectedProduct}
        isOpen={isModalOpen}
        onClose={handleCloseModal}
      />
    </main>
  );
}